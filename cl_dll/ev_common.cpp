...
void EV_EjectBrass(float* origin, float* velocity, float rotation, int model, int soundtype)
{
	...
	VectorClear(endpos);
	endpos[1] = rotation;
	gEngfuncs.pEfxAPI->R_TempModel(origin, velocity, endpos*12, 30, model, soundtype);
}
...
Bullet shell casings w/ spin
=================
*/
void EV_EjectBrassSFS(float* origin, float* velocity, float rotation, int model, int soundtype)
{
	Vector endpos;
	VectorClear(endpos);
	endpos[1] = rotation;
	TEMPENTITY* entity = gEngfuncs.pEfxAPI->R_TempModel(origin, velocity, endpos, 30, model, soundtype);
}
...
void EV_GetDefaultShellInfo(event_args_t* args, float* origin, float* velocity, float* ShellVelocity, float* ShellOrigin, float* forward, float* right, float* up, float forwardScale, float upScale, float rightScale)
{
	...
	fR = gEngfuncs.pfnRandomFloat(100, 150);
	fU = gEngfuncs.pfnRandomFloat(100, 125);
