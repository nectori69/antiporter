...
void CShotgun::Precache()
{
	...
	m_iShell = PRECACHE_MODEL("models/shells/12ga.mdl"); // shotgun shell

	PRECACHE_SOUND("items/9mmclip1.wav");

	PRECACHE_SOUND("weapons/spas12/spas_firesingle.wav"); //shotgun
	PRECACHE_SOUND("weapons/spas12/spas_firedouble.wav"); //shotgun
	
	PRECACHE_SOUND("weapons/spas12/spas_reload1.wav"); // shotgun reload
	PRECACHE_SOUND("weapons/spas12/spas_reload2.wav"); // shotgun reload

	PRECACHE_SOUND("weapons/spas12/spas_pump.wav"); // shotgun pump
...
void CShotgun::Reload()
{
	...
	else if (m_fInSpecialReload == 1)
	{
		...
		if (RANDOM_LONG(0, 1))
			EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/spas12/spas_reload1.wav", 1, ATTN_NORM, 0, 85 + RANDOM_LONG(0, 0x1f));
		else
			EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/spas12/spas_reload2.wav", 1, ATTN_NORM, 0, 85 + RANDOM_LONG(0, 0x1f));
	...
	if (m_flTimeWeaponIdle < UTIL_WeaponTimeBase())
	{
		...
		else if (m_fInSpecialReload != 0)
		{
			...
			else
			{
				...
				// play cocking sound
				EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/spas12/spas_pump.wav", 1, ATTN_NORM, 0, 95 + RANDOM_LONG(0, 0x1f));
				m_fInSpecialReload = 0;
