...
LINK_ENTITY_TO_CLASS(weapon_duke4pistol, CDuke4Pistol)

void CDuke4Pistol::Spawn()
{
	// Define the classname of the entity
	// This is the name you should use to reference this entity name in your code base.
	pev->classname = MAKE_STRING("weapon_duke4pistol");
	...
	// Set the weapon ID
	m_iId = WEAPON_DUKE4PISTOL;

	// Tell the engine about the weapon's world model
	SET_MODEL(ENT(pev), "models/w_duke4_pistol.mdl");

	// Set the default ammo value for the weapon
	m_iDefaultAmmo = DUKE4PISTOL_DEFAULT_GIVE;
...
void CDuke4Pistol::Precache()
{
	// Precache models
	PRECACHE_MODEL("models/v_duke4_pistol.mdl");
	PRECACHE_MODEL("models/w_duke4_pistol.mdl");
	PRECACHE_MODEL("models/p_duke4_pistol.mdl");
	PRECACHE_MODEL("models/shells/50ae.mdl");

	// Precache sounds
	PRECACHE_SOUND("weapons/duke4/duke4_pistol_fire.wav");
	PRECACHE_SOUND("weapons/duke4/duke4_pistol_fire_hp.wav");

	// Precache fire event
	m_usFireDuke4Pistol = PRECACHE_EVENT(1, "events/duke4pistol.sc");
}

bool CDuke4Pistol::GetItemInfo(ItemInfo* p)
{
	...
	p->iMaxClip = DUKE4PISTOL_MAX_CLIP;
	...
	// The "position" in the HUD that the weapon is added to. We'll put this after the m92 (which is in slot 3)
	p->iPosition = 4;

	// Set the ID and auto-switching weights of the weapon
	p->iId = m_iId = WEAPON_DUKE4PISTOL;
	p->iWeight = DUKE4PISTOL_WEIGHT;

	return true;
}

void CDuke4Pistol::SecondaryAttack()
{
	// sorry, nothing
}

void CDuke4Pistol::PrimaryAttack()
{
...
void CDuke4Pistol::Fire(float flSpread, float flCycleTime)
{
	...
	PLAYBACK_EVENT_FULL(flags, m_pPlayer->edict(), m_usFireDuke4Pistol, 0.0, (float*)&g_vecZero, (float*)&g_vecZero, vecDir.x, vecDir.y, 0, 0, (m_iClip == 0) ? 1 : 0, 0);
...
bool CDuke4Pistol::Deploy()
{
	TriggerReleased = false;

	return DefaultDeploy(
		"models/v_duke4_pistol.mdl",				// Weapon view model
		"models/p_duke4_pistol.mdl",				// Weapon player model
		DUKE4_PISTOL_DRAW,						// "Draw" animation index for the view model
...
void CDuke4Pistol::Holster()
{
	...
	SendWeaponAnim(DUKE4_PISTOL_HOLSTER);
}

void CDuke4Pistol::Reload()
{
	...
	if (m_iClip == 0)
		iResult = DefaultReload(DUKE4PISTOL_MAX_CLIP, DUKE4_PISTOL_RELOAD, 1.5);
	else
		iResult = DefaultReload(DUKE4PISTOL_MAX_CLIP, DUKE4_PISTOL_RELOAD_NOT_EMPTY, 1.5);
...
void CDuke4Pistol::WeaponIdle()
{
	...
	if (flRand <= 0.3)
	{
		...
		iAnim = DUKE4_PISTOL_IDLE1;
		m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + (60 / 30.0);
	}
	else if (flRand <= 0.6)
	{
		iAnim = DUKE4_PISTOL_IDLE2;
		m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + (60 / 30);
	}
	else if (flRand <= 0.9)
	{
		iAnim = DUKE4_PISTOL_IDLE3;
		m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + (60 / 30.0);
	}
	else
	{
		iAnim = DUKE4_PISTOL_FIDGET;
